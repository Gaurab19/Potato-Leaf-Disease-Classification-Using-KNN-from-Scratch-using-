...
def loadDataset(filename, trainingSet=[] ,trainingClass=[]):
    arr=[]
    names = ['sumValue','contrast', 'dissimilarity', 'homogeneity', 'ASM', 'energy','hue','value', 'saturaton','label']
...
def knn(trainingSet ,trainingClass,features,k):
    ...
    if(healthy_count >= unhealthy_count):
        ...
        if(early_count >= late_count):
            diagno="Early_Blight Remedies:Burn or bag infected plant parts. Do NOT compost. Drip irrigation and soaker hoses can be used to help keep the foliage dry."
        else:
            diagno="Late_Blight  Remedies: # Monitor the field, remove and destroy infected leaves. # Treat organically with copper spray. # - Use chemical fungicides,the best of which for potato is chlorothalonil."
    return diagno
